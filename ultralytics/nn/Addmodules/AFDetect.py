...
class AFDetect(Detect):
    def __init__(self, nc=80, ch=(), use_fusion=True):
        ...
        if self.use_fusion:
            self.cls_attention = nn.ModuleList(nn.Sequential(Conv(c3, c2, 1), nn.Sigmoid()) for _ in ch)
            self.reg_attention = nn.ModuleList(nn.Sequential(Conv(c2, c3, 1), nn.Sigmoid()) for _ in ch)
